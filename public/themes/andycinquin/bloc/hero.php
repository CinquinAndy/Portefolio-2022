<section class="p-8 xl:p-20 pt-[300px] w-full ">
    ...
    <div class="flex justify-between">
        ...
        <div class="flex justify-end items-end w-1/2">
            <button onclick="location.href = '/contact'"
                    class="px-16 py-8 xl:text-sm text-xl bg-indigo-600 rounded xl:px-10 xl:py-4">Me contacter</button>
        </div>
    </div>
    <div class="flex overflow-hidden flex-row flex-nowrap mt-20">
        <div class="flex flex-row flex-nowrap gap-[40px] animate-scrolling">
            ...
            <?php foreach ($service_repeater as $service): ?>
                <section class="flex flex-col w-[600px] h-[500px] xl:h-[350px] p-20 bg-slate-1000">
                    <div class="flex justify-center items-center w-full">
                        <img src="<?= esc_url($service['img']['url']) ?>" alt="<?= esc_attr($service['img']['alt']) ?>"
                             class="w-[100px] h-[100px] xl:w-[75px] xl:h-[75px]">
                    </div>
                    <header class="flex justify-center items-center py-10 w-full xl:py-6">
                        <h2 class="text-3xl normal-case xl:text-xl">
                            <?= preg_replace('/\*([^*]+)\*/', '<span class="text-indigo-500 font-display">\1</span>', $service['title']) ?>
                        </h2>
                    </header>
                    <article class="text-lg xl:text-sm">
    ...
    <div class="flex justify-between mt-[300px]">
        ...
        <div class="flex flex-col items-end w-1/2 xl:flex-row xl:justify-end">
            <button onclick="location.href = '/contact'"
                    class="px-16 py-8 xl:text-sm text-xl bg-indigo-600 rounded xl:px-10 xl:py-4 mb-6 xl:mb-0 xl:mr-6">
                Me contacter
            </button>
            <button onclick="location.href = '/mes-realisations'"
                    class="px-16 py-8 xl:text-sm text-xl bg-sky-600 rounded xl:px-10 xl:py-4">Mes réalisations
            </button>
        </div>
    </div>
    <div class="flex overflow-hidden flex-row flex-nowrap mt-20">
        <div class="flex flex-row flex-nowrap gap-[40px] animate-scrolling-rea">
            ...
            <?php foreach (get_posts() as $post): ?>
                <a href="<?= esc_url(get_permalink(get_posts()[0])) ?>"
                   class="flex flex-col w-[600px] h-[500px] xl:h-[350px] p-14 pb-4 relative">
                    <h2 class="absolute top-0 left-0 z-30 w-2/3 text-4xl font-black normal-case xl:text-3xl">
                        <?= $post->post_title ?>
                    </h2>
                    <div
                    ...
                    <h2 class="absolute bottom-0 left-0 z-30 text-4xl font-black text-sky-400 xl:text-3xl xl:font-bold">
